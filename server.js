...
function runRobot(
  shouldShutdown = process.env.AUTO_SHUTDOWN === 'true',
  isManual = false
) {
  ...
  child.on('exit', async code => {
    ...
    logSessionSeparator('FIM DA SESSÃO')
...
async function runCleanup() {
  try {
    ...
    if (!supabase) {
      console.log('Limpeza no Supabase ignorada: Supabase não configurado.')
      return
    }
    ...
    if (logError)
      console.error(
        'Erro ao limpar logs antigos do Supabase:',
        logError.message
      )
    else
      console.log(
        `Limpeza de logs antigos do Supabase concluída (retenção: ${retentionDays} dias).`
      )

    const { data: cleanedRows, error: historyError } = await supabase.rpc(
      'cleanup_old_run_results',
      { keep_count: 30 }
    )
    if (historyError)
      console.error(
        'Erro ao limpar histórico de execuções:',
        historyError.message
      )
    else
      console.log(
        `Limpeza de histórico de execuções concluída: ${cleanedRows ?? 0} registro(s) removido(s) (mantendo 30 por conta).`
      )
  ...
  try {
    ...
    for (const data of monthlyData) {
      ...
      await supabase
        ...
        .upsert(payload, { onConflict: 'year,month,account_key' })
    }
